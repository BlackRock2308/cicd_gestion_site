...
class TypeResourceTest extends BasicResourceTest {

    static private TypeVM vm;
    @Autowired
    private ITypeService service;
   static private TypeDTO dto;
    ...
    void findAll_shouldReturnTypes() throws Exception {
        dto = service.save(vm);
        mockMvc.perform(get(UrlMapping.Type.ALL)
    ...
    void findById_shouldReturnType() throws Exception {
        dto = service.save(vm);
        mockMvc.perform(get(UrlMapping.Type.FIND_BY_ID, dto.getId())
                .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
                //.andExpect(jsonPath("$.[0].id").exists())
    ...
    void findById_withBadId_shouldReturnNotFound() throws Exception {
        mockMvc.perform(get(UrlMapping.Type.FIND_BY_ID, UUID.randomUUID().toString())
                .accept(MediaType.APPLICATION_JSON))
    ...
    void add_withNameMaxLengthExceeded_shouldReturnBadRequest() throws Exception {
        ...
        mockMvc.perform(post(UrlMapping.Type.ADD)
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                //.content(q)
                )
    ...
    void update_shouldUpdateType() throws Exception {
        ...
        dto = service.save(vm);
        mockMvc.perform(
                put(UrlMapping.Type.UPDATE, dto.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(TestUtil.convertObjectToJsonBytes(vm))
        )
    ...
    void update_withNameMinLengthExceeded_shouldReturnBadRequest() throws Exception {
        dto = service.save(vm);
        vm.setName(RandomStringUtils.random(SizeMapping.Name.MIN - 1));
        mockMvc.perform(put(UrlMapping.Type.UPDATE, dto.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(vm))
                )
    ...
    void delete_shouldDeleteType() throws Exception {
        dto = service.save(vm);
        mockMvc.perform(
                delete(UrlMapping.Type.DELETE, dto.getId())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void delete_withBadId_shouldReturnNotFound() throws Exception {
        dto = service.save(vm);
        mockMvc.perform(
                delete(UrlMapping.Type.DELETE, UUID.randomUUID().toString())
                        .contentType(MediaType.APPLICATION_JSON)
        ).andExpect(status().isNotFound());
    }
