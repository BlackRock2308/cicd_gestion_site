pipeline {
    ...
    stages {
        ...
        stage("Maven Build") {
            steps {
                script {
                    bat "mvn clean build install -DskipTests=true"
                }
            }
        }
    ...
    stage('SonarQube analysis') {
        ...
        steps{
            script{
                deploy adapters: [tomcat9(credentialsId: 'tomcat_credentials', path: '', url: 'http://localhost:8082')], contextPath: 'sample', onFailure: false, war: '**/*.war'
            }
        }
    }
    }
